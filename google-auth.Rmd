...
your identity. In this chapter, we will walk through the process of
...
6) Add encryption key to RStudio Connect or Github as an environment variable
...
3) Encryption keys or the passwords used to generate them are stored as environment variables and retrieved from `.Renviron`, never hard coded into scripts. 
...
So now you have this super secret key for the service account and need to securely access it in an R project. We can use the `sodium`, `gargle`, and `cyphr` packages to encrypt the JSON file that stores our key, safely store the encrypted file in the R project, and securely store the encryption key as an environment variable. If additional files or data require encryption it is recommended that use the `git-crypt` approached described in chapter \@ref(encryption). 
...
- The `.Renviron` file has to end with a single return 
...
Now that we have a password stored as an environment variable, we can symmetrically encrypt our JSON file that stores the service account key. The encrypted file will live in `inst/tokens` within your working directory. After the file has been encrypted it is safe to push it to a remote git repository. 

```{r eval=FALSE}

get_sodium_key <- function(env_var_name = "SERVICE_ACCOUNT_PASSWORD"){
...
cyphr::encrypt_file(path = "My/directory/outside/the/project/longComplicatedFile.json",dest = "./inst/tokens/longComplicatedFile.json", key = key)

# The encrypted file in inst/tokens is now safe to push to a remote repository

# Note - sodium keys can be converted from binary to hex and stored as environment varaibles. This is a convenient way to use the sodium key if you prefer to not deal with passwords. 

hex_key <- sodium::bin2hex(key$key())

# add to environment variables as above
# retrieve from environment variables as

bin_key <- sodium::hex2bin(hex_key)
...
Now we will decrypt the credentials then pass them to Google using the `gs4_auth` function.
...
So now you have this super secret key for the service account but need to make it accessible to your R package. We can use the `sodium` package as well as functions in `gargle` to encrypt the JSON file that stores our key and securely store the encryption key as an environment variable.  Notice that we are using the `:::` operator in function calls. See [this article](https://gargle.r-lib.org/articles/articles/managing-tokens-securely.html) for more details.
