...
If you setup `git-crypt`, make sure the file is saved in an appropriate location (e.g. `./auth`) and that all the users who need to use the encrypted file are [added to the repo][Allow contributors access to encrypted files]. If you used `git-crypt`, you can skip ahead to [Run your code using encrypted keys](#run-your-code-using-encrypted-keys).  
