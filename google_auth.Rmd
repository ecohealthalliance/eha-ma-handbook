...
store and collaborate on files. Working with googles drive based files in R is 
...
We are going to focus on setting up an automate workflow in R for google sheets
...
Basically we want to either provide a token directly or make a token discoverable. 
...
- In the leftside, navigate to APIs& Services > Library 
...
R project. We can use the `sodium` package as well as functions in `gargle` to 
password protect the JSON file that stores our key. We can then store the password
as an environmental variable in the project. Notice that we are using the `:::` in
function calls. 
...
***KEEP THIS SECRET IN YOUR PROJECT*** 
...
**TODO**: expand to include git-crypt example or use of github secrets to store json in remote location.

```{r eval=FALSE}
library(googlesheets4)
gs4_auth(path = "keybase://private/myname/longComplicatedFile.json")
...
read_sheet("https://docs.google.com/spreadsheets/that/i/definitely/shared/with/the/service-account")
