...
repos for encrypted sharing using `gpg`.

These are already installed on EHA servers.  
...
The first appraoch is to utilize the Windows Subsystem for Linux (WSL). This method requires Windows 10 or higher.  You must first install WSL.  Do so following this guide: <https://docs.microsoft.com/en-us/windows/wsl/install>. 
...
The second method is to install pre-compiled binaries for GPG and git-crypt.

First 0 A Windows-compatible binary for GPG is available at  <https://gnupg.org/download/>. The 'Simple installer for the current GnuPG' binary on that page is the recommended choice.
...
Once Keybase and GPG are installed, the terminal commands related to exporting keys from Kebyase into GPG are the same regardless of operating system.
...
In the nano edior, press Ctrl-O to write ("write-Out") to save your changes, then press Ctrl-X to exit

```

Alternatively, run the following line in the terminal to change your `.zshrc file without using nano or any other editor:
...
Windows need to inform git of the location of the gpg executable. This can be done by opening cmd or PowerShell and entering the following command: 
